...
depth of $\tauh=10^7$.
The results of the semi-analytic model for $\tauh=10^6$ and
$\tauh=10^7$ can in turn be approximated by a gaussian smoothing with
standard deviation $\sigma_v = \vrot\sin(\theta)/2$. 
...
this smoothing can also be a natural consequence of the intruments
used to measure the LAE spectra.
In other words, poor spectral resolution could be mistaken as an
indication for rotation.

We model the effects of gaussian smoothing with a similar ansatz as
the semi-analytic model by changing the frequency of each photon by a
Gaussian random variable centered on zero and width $\sigma_x$:
\begin{equation}
x^{\prime} = x + \mathcal{N}(0,\sigma_x).
\end{equation}
...
This approximation works well to reproduce the semi-analytic results
for $\tau=10^6$ and $\tau=10^7$.
...
%When located in the regimes of low optical thickness, \tauh $\lesssim
%4$,  the agreement of the radiative transfer simulation with the
%semi-analytical solution is not as accurate. In this regime, only
%numerical simulations can help to predict the shape of the \lya
%profiles. We ran a simulation with low \tauh and compared it with the
%semi-analytical model. As seen in Figure \ref{fig:low_tau}, the model
%does not fit the simulation. This creates physical restrictions on the
%input of our model. 
