...
kinematic studies that can be performed on galaxies with small angular
sizes on the sky.
For instance, spatially resolved spectra on dwarf galaxies show signs
of coherent rotation \citep{2009A&A...493..871S}.
...
Some of those star forming galaxies are expected to have high neutral
gas contents and low dust contents and thus show \lya\ emission with
rotation imprints
\citep{2005A&A...433L...1B,2008ApJ...672..888T,2013MNRAS.434.2491G} 

The computational \lya modeling of systems showing rotation has only
been studied until recently by \cite{Garavito14}.
In that work the authors studied the influence of pure solid body
rotation on the \lya line's morpholgy.
They found that rotation indeed introduces changes, the most
noticeable change is the dependence of the spectra with the viewing
...
Here we present for the first time such as study. 
...
\begin{figure}
  ...
  \end{center}
  \caption{\textbf{Comparison between observations and simulations.}  
    \label{fig:muse}}
\end{figure}
...
Doppler shift when taking spectra at two oposite sides of a LAE.
...
\subsubsection{Relationship to MUSE observations}

In order to give the reader a feeling of the ranges of observational
data that can be explained by the models presented here we perform a
comparison against observational data from MUSE-Wide survey (MWS)
\citep{2017A&A...606A..12H}. 

MWS provides public access to 831 spectra out of which 237 contain a
detected \lya line as dominante. 
The public dataset reports as the redshift of the galaxy the position
of the dominant peak.
We use this redshift to convert the spectra to rest-frame and compute
the standard deviation and skewness statitstics. 
Given the uncertainty on the galaxy redshift we do not compute the
valley/peak ratio. 
We do not try to perform a full fit to all the spectra but only to
explore whether the region of parameter space explored in our RT
simulations shows any crossing with the results from the MWS.
...
We find that the main effects of rotation on the \lya\ line morphology are:
...
As an application to observational data our results suggest that
rotation should also be considered as an explanation of the
non-vanishing \lya intensity at the line's center.
We also show how the \lya spectra from two different sides of a galaxy
can detect approaching/receeding gas motions We show that the
distances between the peaks of these two spectra is smaller than the
expected value of $2\times\vrot$ due to the differente weights at the
surface of the emitting regions.  
Finally we also use data from MUSE to demonstrate to what extent the
outputs of the RT results presented here can be expected to match
hundreds of observed \lya spectra.
...
influence into already existing RT simulations. 
